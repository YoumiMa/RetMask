...
from faiss_attn.source.modeling_qwen3 import Qwen3ForCausalLM
from faiss_attn.source.modeling_olmo3 import Olmo3ForCausalLM
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if "Qwen2" in self.model_version:
            self.model_to_test = Qwen2ForCausalLM.from_pretrained(
                    model_name,dtype="auto",device_map='auto',attn_implementation="flash_attention_2"
                ).eval()
        elif "Qwen3" in self.model_version:
            if "A" in self.model_version:
                self.model_to_test = Qwen3MoeForCausalLM.from_pretrained(
                 model_name,dtype="auto",device_map='auto',attn_implementation="flash_attention_2"
                ).eval()                
            else:
        ...
        elif "Olmo-3" in self.model_version:
            self.model_to_test = Olmo3ForCausalLM.from_pretrained(
                 model_name,dtype="auto",device_map='auto',attn_implementation="flash_attention_2"
                ).eval()
        elif "Mixtral" in self.model_version:
            self.model_to_test = MixtralForCausalLM.from_pretrained(
                    model_name,dtype="auto",device_map='auto',attn_implementation="flash_attention_2",trust_remote_code=True,
                ).eval()
            ...
            self.model_to_test = LlamaForCausalLM.from_pretrained(model_name,
                attn_implementation="flash_attention_2", dtype=torch.bfloat16,device_map='auto').eval()
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        '''
        if self.model_version in ["Mistral-7B-Instruct-v0.2", "Qwen1.5-14B-Chat", "Llama-3.1-8B-Instruct", "Llama-3.1-70B-Instruct", "Qwen3-8B", "Qwen3-32B", "Qwen3-30B-A3B", "Olmo-3-7B-Think", "Olmo-3-7B-Instruct"]:
            if self.enc.bos_token:
                context = context.replace(self.enc.bos_token, "")
            prompt = [
            {"role": "user", "content": f"<book>{context}</book>\nBased on the content of the book, Question: {self.retrieval_question}\nAnswer:"},
            ]
            if "Qwen3" in self.model_version:
                input_ids = self.enc.apply_chat_template(conversation=prompt, tokenize=True,  add_generation_prompt=True, return_tensors='pt', enable_thinking=False)
            else:
        ...
        if score > 50:
            ...
            head_score = [(i[0], np.mean(i[1])) for i in self.head_counter.items()] # average over all scores
